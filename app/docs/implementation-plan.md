...
| Delivered | **A browser has now run the handheld** — `tools/rf-bench/`. Since it shipped, the add-in's script was the one thing in the app nothing could check: no test runner, no container to publish to, and a container credential this project does not have. The bench loads the **real** `rfterminal.js` and `rfterminal.css` from where they ship, stubs the one Business Central global they touch, feeds them state documents in the shape `WHA RF Terminal State` builds, and asserts on the events they raise — 37 assertions across a wedge matrix, four screen sizes, focus, the disabled keys, label suppression off the simulator, and markup in a state document. It needs no AL compile, so it is also **the first thing in this repository that can run in CI**, which the AL build still cannot without private `bc-dev-templates` access. The same page served over the LAN is the device half: the real screen, the real scanner, the real wedge configuration. It **emulates no make of handheld** and the profiles are named after characteristics for that reason. The fixture values are hand-derived from the AL and have never been produced by a running Business Central |
...
| Delivered | **The movement worksheet as a task source — and the finding that it is the last one.** Business Central's internal put-away, internal pick and production picks all test the location for `Require Put-away`, `Require Pick` or `Directed Put-away and Pick` before they will exist, and every such location is one this app refuses to raise work in. Those four sources are **unreachable, not unbuilt**; adding them would be code that can never run. The movement worksheet needs only `Bin Mandatory`, so it is reachable, and its line is already the exact shape of a warehouse task. See [location-configuration.md](location-configuration.md) |
| Delivered | **The app prints.** Three reports — handling unit contents, the task list, and a count sheet — the first report objects in the app, which had shipped none, each with an RDLC layout in `app/layout/`. The compiler catches a layout that is **broken** and misses one that is merely **wrong**: malformed XML is `AL0444`, but a reference to a field that does not exist renders blank in silence, and a path to a missing file is not an error at all — `alc` generates a layout there instead. `tools/rdlc-check/` is the check that was missing, and it found a real discrepancy on its first run — which is how the compiler's silent rewriting of the layout dataset was discovered rather than assumed. The blind-count suppression is not in the report: a layout that leaked the expected quantity would defeat that feature silently, so the report asks and does not decide. See [reports.md](reports.md) |
| Decided | **The scope fork, taken** — [scope-fork.md](scope-fork.md). The app kept a queue *beside* Business Central's `Warehouse Activity Line` and was therefore excluded from every location that raises one: directed put-away and pick, production, assembly, internal warehouse documents. That is the shape of the app, not a missing feature, and it excluded the app from the warehouses that most need one. Recommendation: **work on top of Business Central's activities rather than beside or instead of them** — Business Central raises the job, this app executes it and registers it. Incremental: a fifth `WHA ITaskSource`, and the 51 files across eight features that read `WHA Warehouse Task` keep working untouched |
| Delivered | **The scope fork, first segment.** A fifth `WHA ITaskSource` reads `Warehouse Activity Line` — Business Central raises the job, this app executes it, and finishing it calls `Whse.-Activity-Register`, the codeunit Business Central's own page action uses. One job is a **take-and-place pair**, not a line, so a task from this source carries both bins, which no document source can give. Warehouse registration steps aside for it, because registering the activity writes the entries itself. **This is the first work the app can do at a directed put-away and pick location** |
...
| Location configuration is discipline, not code, beyond one guard | Two queues over the same stock, or a write-back that fails on every line | [location-configuration.md](location-configuration.md). The document sources refuse a location that raises its own activities; nothing checks a location changed afterwards, and nothing checks work created by hand |
...
1. **Run the test suite once.** 393 automated tests exist across seventeen test codeunits and **not one
