...
| Delivered | **The app prints.** Three reports — handling unit contents, the task list, and a count sheet — the first report objects in the app, which had shipped none. **They carry no layout file**, deliberately: Business Central draws a built-in one and a customer replaces it through Report Layouts, which keeps the only part of this repository no compiler checks and no test reads out of the product entirely. The blind-count suppression is not in the report either — a layout that leaked the expected quantity would defeat that feature silently, so the report asks and does not decide. See [reports.md](reports.md) |
